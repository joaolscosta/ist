...
;
; **********************************************************************
desenha_meteoro_inicial:					; desenha o meteoro ou a nave na primeira linha do ecrã na sua forma inicial
	PUSH R2
	PUSH R3
	PUSH R4
	PUSH R5
	...
	PUSH R7
	MOV  R7, 1
	MOV  [sub_met], R7						; atualizar o valor da variavel com 1 pois corresponde à primeira altura
	MOV  [fase_meteoro], R7					; atualizar o valor da variavel da fase correspondente à primeira
	POP  R7
	MOV  R4, DEF_METEORO_MIN				; escolher a tabela correspondente ao formato da primeira fase
	JMP  gera_aleatoriamente			

gera_aleatoriamente_pop:					; vai repetir o ciclo gera_aleatoriamente de forma a não colidir com nada
	MOV R7, 0
	MOV [vai_colidir], R7					; repor o valor da variavel
	POP R7

gera_aleatoriamente:						; vai gerar o valor de uma coluna aleatoriamente no ecrâ
	MOV R9, [PIN]							; o valor do periferico PIN fica guardado no registo R9
	SHR R9, 5								; fazer SHR cinco vezes para aceder aos bits que não estão preenchidos,
											; obtendo um valor aleatorio			
	CALL subtrair_64						
	PUSH R7
	PUSH R6
	PUSH R5
	PUSH R9
	PUSH R8
	MOV  R8, R9								; guardar o novo valor da coluna no registo R8
	MOV  R6, 0								; valor a ser incrementado no limites_novo_direita
	MOV  R5, 0								; valor a ser incrementado no limites_novo_direita
	CALL limites_novo_direita				; verificar se do lado direito do ecra e em baixo existe algum objeto desenhado
	POP  R8
	POP  R9
	PUSH R9
	PUSH R8
	MOV  R8, R9
	MOV  R6, 7								; valor a ser decrementado no limites_novo_esquerda
	MOV  R5, 7								; valor a ser decrementado no limites_novo_esquerda
	CALL limites_novo_esquerda				; verificar se do lado esquerdo do ecra e em baixo existe algum objeto desenhado
	POP  R8
	POP  R9
	POP  R5
	POP  R6
	MOV  R7, [vai_colidir]					; guardar o valor da variavel no registo R7
	CMP  R7, 1
	JZ   gera_aleatoriamente_pop			; se na coluna gerada for colidir com algum objeto vai para o gera_aleatoriamente_pop
	POP  R7
	MOV  R5, [R4]                			; obtém a largura do meteoro
	ADD  R4, 2                    			; muda de linha da tabela pra obter a altura 
	MOV  R10, [R4]              			; obtém a altura do meteoro
	ADD  R4, 2                    			; endereço da cor do 1º pixel (2 porque a largura é uma word)
	MOV  [coluna_atual_meteoro], R9			; atualiza o valor da coluna atual do meteoro com o novo valor gerado aleatoriamente
	MOV  R2, R9								; muda o valor da coluna
	POP  R9
	PUSH R1
	POP  R1
	JMP  desenha_pixels_meteoro				; vai desenhar os pixeis do meteoro

desenha_meteoro:                            ; desenha o meteoro e as naves
	PUSH R2
	MOV  R2, [coluna_atual_meteoro]         ; obtem o valor da coluna do meteoro/nave que vamos desenhar
	PUSH R3
	PUSH R4
	PUSH R5
	PUSH R8
	...
	MOV  R7, [fase_meteoro]                 ; obtem a fase do meteoro/nave a desenhar
	CMP  R7, 3                              ; ve se está na fase inicial, movimentou-se menos de 3 vezes
	JLT  continua_desenha_min               ; está na fase inicial

	CMP  R7, 6                              ; ve se está na fase 2, movimentou-se menos de 6 vezes
	JLT  continua_desenha2                  ; está na fase 2

	PUSH R9
	MOV  R9, 9                              ; movimentou-se menos de 9 vezes
	CMP  R7, R9                             ; vê se está na fase 3
	JLT  continua_desenha3                  ; está na fase 3

	MOV  R9, 12                             ; movimentou-se menos de 12 vezes
	CMP  R7, R9                             ; vê se está na fase 4
	JLT  continua_desenha4                  ; está na fase 4
                                            
	POP  R9                                 ; não está em nenhuma das fases anteriores
	JMP  continua_desenha_max               ; está na fase de tamanho máximo

continua_desenha_min:                       ; desenha o meteoro/nave na fase inicial
	SUB R1, 1                               ; subtraimos a altura do meteoro/nave nesta fase
	MOV  R7, 1                          
	MOV  [sub_met], R7                      ; está na fase 1, subtrai 1 linha, ou seja, a altura do meteoro/nave nesta fase
	POP  R7
	MOV  R4, DEF_METEORO_MIN                ; endereço da tabela que define o meteoro/nave nesta fase
	JMP  continua_desenha			        ; vai desenhar o meteoro/nave

continua_desenha2:                          ; desenha o meteoro/nave na fase 2
	SUB R1, 2                               ; subtraimos a altura do meteoro/nave nesta fase
	MOV  R7, 2
	MOV  [sub_met], R7                      ; está na fase 2, subtrai 2 linhas, ou seja, a altura do meteoro/nave nesta fase
	POP  R7
	MOV  R4, DEF_METEORO_2                  ; endereço da tabela que define o meteoro/nave nesta fase
	JMP  continua_desenha					; vai desenhar o meteoro/nave

continua_desenha3:                          ; desenha o meteoro/nave na fase 3
	SUB R1, 3                               ; subtraimos a altura do meteoro/nave nesta fase
	POP  R9
	MOV  R7, 3                              
	MOV  [sub_met], R7                      ; está na fase 3, subtrai 3 linhas, ou seja, a altura do meteoro/nave nesta fase
	MOV R7, [seleciona_meteoro]
	CMP R7, 1                               ; vê se o meteoro está selecionado 
	JZ continua_desenha3_met                ; é o meteoro que está selecionado

seleciona_nave_desenhar:                    ; não estamos a desenhar o meteoro, vê qual das naves estamos a desenhar
	PUSH R9
	MOV R7, 1
	MOV R9, [seleciona_nave]                ; obtem o numero da nave selecionada
	CMP R7, R9                              ; vê se é a nave 1
	JZ desenha_nave1_fase3                  ; é nave 1
	MOV R7, 2
	CMP R7, R9                              ; vê se é a nave 2
	JZ desenha_nave2_fase3                  ; é nave 2
	MOV R7, 3                               
	CMP R7, R9                              ; vê se é a nave 3
	JZ desenha_nave3_fase3                  ; é nave 3

desenha_nave1_fase3:                        ; desenha a nave 1 na fase 3
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE_3                     ; endereço da tabela que define a nave 1 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

desenha_nave2_fase3:                        ; desenha a nave 2 na fase 3
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE1_3                    ; endereço da tabela que define a nave 2 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

desenha_nave3_fase3:                        ; desenha a nave 3 na fase 3
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE2_3                    ; endereço da tabela que define a nave 3 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

continua_desenha3_met:
	POP  R7
	MOV  R4, DEF_METEORO_3                  ; endereço da tabela que define o meteoro nesta fase
	JMP  continua_desenha					; vai desenhar o meteoro

continua_desenha4:                          ; desenha o meteoro/nave na fase 4
	SUB R1, 4								; subtraimos a altura do meteoro/nave nesta fase
	POP  R9
	MOV  R7, 4	
	MOV  [sub_met], R7						; está na fase 4, subtrai 4 linhas, ou seja, a altura do meteoro/nave nesta fase
	MOV R7, [seleciona_meteoro]
	CMP R7, 1								; vê se o meteoro está selecionado 
	JZ continua_desenha4_met				; é o meteoro que está selecionado

seleciona_nave_desenhar_fase4:				; não estamos a desenhar o meteoro, vê qual das naves estamos a desenhar
	PUSH R9
	MOV R7, 1
	MOV R9, [seleciona_nave]				; obtem o numero da nave selecionada
	CMP R7, R9								; vê se é a nave 1
	JZ desenha_nave1_fase4					; é nave 1
	MOV R7, 2
	CMP R7, R9								; vê se é a nave 2
	JZ desenha_nave2_fase4					; é nave 2
	MOV R7, 3
	CMP R7, R9								; vê se é a nave 3
	JZ desenha_nave3_fase4					; é nave 3

desenha_nave1_fase4:						; desenha a nave 1 na fase 4
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE_4						; endereço da tabela que define a nave 1 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

desenha_nave2_fase4:						; desenha a nave 2 na fase 4
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE1_4					; endereço da tabela que define a nave 2 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

desenha_nave3_fase4:						; desenha a nave 3 na fase 4
	POP  R9
	POP  R7	
	MOV  R4, DEF_NAVE2_4					; endereço da tabela que define a nave 3 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

continua_desenha4_met:						; desenha o meteoro na dase 4
	POP  R7
	MOV  R4, DEF_METEORO_4					; endereço da tabela que define o meteoro nesta fase
	JMP  continua_desenha					; vai desenhar o meteoro

continua_desenha_max:                       ; desenha o meteoro/nave na fase do seu tamanho máximo
	SUB R1, 5								; subtraimos a altura do meteoro/nave nesta fase
	MOV  R7, 5
	MOV  [sub_met], R7						; está na fase máximo, subtrai 5 linhas, ou seja, a altura do meteoro/nave nesta fase
	MOV R7, [seleciona_meteoro]
	CMP R7, 1								; vê se o meteoro está selecionado 
	JZ continua_desenha_max_met				; é o meteoro que está selecionado

seleciona_nave_desenhar_fase5:				; não estamos a desenhar o meteoro, vê qual das naves estamos a desenhar
	PUSH R9
	MOV R7, 1
	MOV R9, [seleciona_nave]				; obtem o numero da nave selecionada
	CMP R7, R9								; vê se é a nave 1
	JZ desenha_nave1_fase5					; é nave 1
	MOV R7, 2
	CMP R7, R9								; vê se é a nave 2
	JZ desenha_nave2_fase5					; é nave 2
	MOV R7, 3		
	CMP R7, R9								; vê se é a nave 3
	JZ desenha_nave3_fase5					; é nave 3

desenha_nave1_fase5:						; desenha a nave 1 na fase máxima do seu tamanho
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE_MAX					; endereço da tabela que define a nave 1 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

desenha_nave2_fase5:						; desenha a nave 2 na fase máxima do seu tamanho
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE1_MAX					; endereço da tabela que define a nave 2 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

desenha_nave3_fase5:						; desenha a nave 2 na fase máxima do seu tamanho
	POP  R9
	POP  R7
	MOV  R4, DEF_NAVE2_MAX					; endereço da tabela que define a nave 3 nesta fase
	JMP  continua_desenha					; vai desenhar a nave

continua_desenha_max_met:					; desenha o meteoro na fase máxima do seu tamanho
	POP  R7
	MOV  R4, DEF_METEORO_MAX				; endereço da tabela que define o meteoro nesta fase
	JMP  continua_desenha					; vai desenhar o meteoro

continua_desenha:
	MOV	 R5, [R4]		        			; obtém a largura do meteoro/nave
	ADD  R4, 2				    			; muda de linha da tabela pra obter a altura 
	MOV  R10, [R4]              			; obtém a altura do meteoro/nave
	ADD	 R4, 2			        			; endereço da cor do 1º pixel (2 porque a largura é uma word)
	PUSH R1
	MOV  [linha_atual_meteoro], R1			; obtem a linha atual do meteoro/nave
	POP  R1

desenha_pixels_meteoro:         			; desenha os pixels do meteoro/nave a partir da tabela
	MOV	 R3, [R4]			    			; obtém a cor do próximo pixel do meteoro/nave
	CALL escreve_pixel_met					; escreve cada pixel do meteoro/nave
	ADD	 R4, 2			        			; endereço da cor do próximo pixel (2 porque cada cor de pixel é uma word)
    ADD  R2, 1                  			; próxima coluna
	SUB  R5, 1 		            			; menos uma coluna para tratar
	JNZ  desenha_pixels_meteoro 			; continua até percorrer toda a largura do meteoro/nave
	ADD  R1, 1								; proxima linha
    MOV  R2, [coluna_atual_meteoro]			; repor o valor da coluna inicial
    MOV  R5, LARGURA_MET					; repor a largura do meteoro/nave para desenhar na proxima coluna
	SUB  R10, 1				    			; menos uma linha para tratar
...
escreve_pixel_met:
	...
	CALL verifica_colisao		 	; verifica se o meteoro colide com algum objeto
...
escreve_pixel_missil:
	MOV  [DEFINE_LINHA], R1			; seleciona a linha
	MOV  [DEFINE_COLUNA], R2		; seleciona a coluna
	CALL verifica_colisao_missil	; verifica se o missil colide com algum objeto
	MOV  [DEFINE_PIXEL], R3			; altera a cor do pixel na linha e coluna já selecionadas
	RET
...
ciclo_atraso:					; quando R11 for zero acaba o atraso
	SUB	 R11, 1
	JNZ	 ciclo_atraso			; volta ao inicio do ciclo
	POP	 R11
	RET
...
converte_numero:			; converte numeros hexadecimais para decimais
	PUSH R3				
	PUSH R4
	PUSH R5
	PUSH R7
	MOV R7, DIVISOR			; vai buscar o valor 10 para dividir pelo numero
...
ciclo_converte:
	...
	CMP R3, R7
	JLT converte_numero_ret ; se for menor sai do ciclo
	JMP ciclo_converte
...
termina_som4:
	PUSH R9
    MOV  R9, 4                    ; som com número 4
    MOV  [TERMINA_SOM], R9        ; comando para terminar o som
	POP R9
	RET
...
move_meteoro:							; vai mover o meteoro ou a nave
    CALL toca_som0						; toca o som de movimento do meteoro
    PUSH R2
    PUSH R4                            	; repor o valor da linha para desenhar o meteoro
    MOV  R2, [coluna_atual_meteoro]     ; guarda o valor da coluna atual no registo R2
...
chegou_a_ultima_linha:                  ; verifica se o limite inferior do meteoro está na ultima linha do ecrã
	...
	PUSH R1
	PUSH R7
	INC  R1								; incrementar o valor da linha
    CMP  R1, R9                         ; compara a linha atual do limite inferior do meteoro com a última linha
    JZ   volta_primeira_linha           ; chegou à ultima linha, por isso vai voltar à primeira
	MOV R9, [seleciona_meteoro]			; guarda o valor que indica se estamos a mover um meteoro ou uma nave
	CMP R9, 1
	JNZ ver_nave 						; se não for um meteoro é uma nave
    MOV R9, [colidiu_met]				; guarda o valor que indica se houve colisao
	CMP R9, 1
	JZ colidiu_meteoro                  ; repomos a linha do meteoro do limite superior se colidiu
	JMP linha_seguinte					; se for um movimento normal, vai move-lo para a linha seguinte

ver_nave:
	MOV R9, [colidiu]					; verifica se colidiu
	CMP R9, 1
	JNZ linha_seguinte					; se não colidiu faz o movimento normal
	MOV R9, [seleciona_nave]			; guarda o valor da variavel no registo R9
	MOV R7, [nave_colidida]				; guarda o valor da variavel no registo R7
	CMP R7, R9							; verifica qual das naves colidiu e qual delas está a ser movimentada
	JNZ linha_seguinte
	JMP volta_primeira_linha			; se colidiu volta à primeira linha

linha_seguinte:							; vai movimentar o meteoro ou a nave para a linha seguinte
	POP R7
	POP R1
	POP  R9
    ADD R1, 2                           ; vamos escrever o meteoro ou a nave duas linhas abaixo
    CALL desenha_meteoro
	POP R4
	POP R2
	RET

colidiu_meteoro:						; se colidiu vamso repor o valor da variavel
	MOV R9, 0
	MOV [colidiu_met], R9
    
volta_primeira_linha:					; vai coltar ao inicio do ecrã
	POP R7
	POP R1
	MOV R9, 0
	MOV [colidiu], R9					; repõe o valor da variavel que indica se houve colisão
    POP R9
	MOV R1, 0                           ; vamos escrever o meteoro ou a nave na primeira linha
	MOV [fase_meteoro], R1				; a variavel que controla a fase do meteoro/ nave passa ao seu valor inicial
	CALL desenha_meteoro_inicial        ; desenha o meteoro/ nave a partir da tabela
	POP R4
	POP R2
	RET
...
verifica_colisao_missil:                        ; verifica se o míssil colide com algo
    ...
    POP R7
    RET
colide_nave:									; vê com qual das naves o missil colide
	PUSH R6
	MOV R6, COR_NAVE1							; obtem a cor da nave 1
	CMP R7, R6									; vê se a cor a nave colidida é a mesma da nave 1
	JZ colide_nave1								; colidiu com a nave 1
	MOV R6, COR_NAVE2							; obtem a cor da nave 2
	CMP R7, R6									; vê se a cor a nave colidida é a mesma da nave 2
	JZ colide_nave2								; colidiu com a nave 1
	MOV R6, COR_NAVE3							; obtem a cor da nave 3
	CMP R7, R6									; vê se a cor a nave colidida é a mesma da nave 3
	JNZ colide_meteoro							; não colidiu com nenhuma das naves, colidiu com o meteoro

colide_nave3:									; colidiu com a nave 3
	MOV R7, 5									; valor da energia a incrementar
	CALL incrementa								; incrementa a energia
	MOV R6, 1									
	MOV [colidiu], R6							; muda para 1 a variavel que indica que houve colisão
	MOV R6, 3									; é a nave 3
	MOV [nave_colidida], R6						; guarda na variavel o numero da nave colidida
	JMP continua_colide							; vai continuar o ciclo da colisão

colide_nave2:									; colidiu com a nave 2
	MOV R7, 5									; valor da energia a incrementar
	CALL incrementa								; incrementa a energia
	MOV R6, 1
	MOV [colidiu], R6							; muda para 1 a variavel que indica que houve colisão
...
colide_nave1:									; colidiu com a nave 1
	MOV R7, 5									; valor da energia a incrementar
	CALL incrementa								; incrementa a energia
	MOV R6, 1
	MOV [colidiu], R6							; muda para 1 a variavel que indica que houve colisão
...
colide_meteoro:									; colidiu com o meteoro
	PUSH R6
	MOV R6, COR_METEORO							; obtem a cor do meteoro
	CMP R7, R6									; vê se a cor do pixel com que colidiu é a mesma da cor do meteoro
	JNZ continua_colide							; nao colidiu com o meteoro
	MOV R6, 1									
	MOV [colidiu_met], R6						; muda para 1 a variavel que indica que houve colisão com o meteoro

continua_colide:
	POP R6
	POP R7
	MOV  R2, [coluna_atual_meteoro]             ; obtem o valor da coluna do meteoro/nave
    MOV  R4, DEF_METEORO_MAX            		; endereço da tabela que define o meteoro
	CALL apaga_meteoro							; vai apagar o meteoro/nave
	MOV R1, [linha_missil]						; obtem o valor da linha em que está o missil
	MOV R2, [coluna_missil]						; obtem o valor da coluna em que está o missil
	SUB  R1, 3									; vai desenhar a explosão 3 linhas acima de onde o missil colidiu
	SUB  R2, 3                					; vai desenhar a explosão 3 colunas à esquerda de onde o missil colidiu
	MOV [linha_missil_explosao], R1				; guarda  valor da linha a começar desenhar a explosão
	MOV [coluna_missil_explosao], R2			; guarda  valor da coluna a começar desenhar a explosão
    MOV  R4, DEF_EXPLOSAO						; endereço da tabela que define a explosão

	PUSH R5
	PUSH R6
	MOV R6, 6									; valor do numero de linhas para desenhar a explosão
	MOV R5, 6									; valor do numero de colunas para desenhar a explosão

desenha_explosao:
	MOV	 R3, [R4]								; obtem a cor do proximo pixel da explosão
	CALL escreve_pixel							; escreve cada pixel da explosão
	DEC R5										; menos uma coluna a desenhar
	ADD R4,2									; endereço da cor do próximo pixel (2 porque cada cor de pixel é uma word)
	INC R2               						; passa para a próxima coluna à direita
	CMP R5, 0									; vê se já desenhou todas as colunas na linha atual
	JNZ desenha_explosao						; continua até percorrer todas as colunas do desenho da explosão
	DEC R6										; menos uma linha a desenhar
	MOV R5, 6									; reinicia o contador de colunas a desenhar 
	SUB R2, 6									; volta à primeira coluna do desenho
	INC R1										; passa para a linha acima
	CMP R6, 0									; vê se já desenhou todas as linhas 
	JNZ desenha_explosao						; continua até desenhar todas as linhas
	POP R6
	POP R5
	MOV R7, 1	
	MOV [explodiu], R7							; muda para 1 a variável que indica se ocorreu  explosão 
	CALL toca_som7								; toca o som da explosão
	POP R7
	JMP disparo_ret								

apaga_explosao:
	MOV R1, [linha_missil_explosao]				; obtem o valor da linha em que foi iniciado o desenho da explosão
	MOV R2, [coluna_missil_explosao]			; obtem o valor da coluna em que foi iniciado o desenho da explosão
	PUSH R5	
	PUSH R6
	MOV R6, 6									; valor do numero de linhas para apagar da explosão
	MOV R5, 6									; valor do numero de colunas para desenhar a explosão

apaga_explosao2:
	MOV	 R3, 0									; cor para apagar o próximo pixel da explosão
	CALL escreve_pixel							; escreve cada pixel da explosão
	DEC R5										; menos uma coluna a apagar
	INC R2               						; passa para a próxima coluna à direita
	CMP R5, 0									; vê se já apagou todas as colunas na linha atual
	JNZ apaga_explosao2							; continua até percorrer todas as colunas do desenho da explosão
	DEC R6										; menos uma linha a apagar
	MOV R5, 6									; reinicia o contador de colunas a apagar
	SUB R2, 6									; volta à primeira coluna do desenho
	INC R1										; passa para a linha acima
	CMP R6, 0									; vê se já desenhou todas as linhas
	JNZ apaga_explosao2							; continua até apagar todas as linhas
	POP R6
	POP R5
	RET
...
subtrair_64:									
	...
	PUSH R7
	MOV R7, 2
	MOV R6, MAX_COLUNA_HEXA						; obtem o numero máximo de colunas em hexadecimal
	CMP R9, R6									; vê se o numero gerado aleatóriamente é maior que o número máximo de colunas
	JGT subtrair								; o número é maior que o número máximo de colunas
	POP R7
	POP R6
	RET

subtrair:										; ciclo para tornar o numero gerado num número até 63 
												; que é o número máximo de colunas
	SUB R9, R6									; subtrair o numero máximo de colunas
	DIV R9, R7									; dividir por um número para o tornar mais aleatório
	INC R7										; incrementar o divisor que torna o número da coluna mais aleatório
	CMP R9, R6									; vê se o numero gerado aleatóriamente é maior que o número máximo de colunas
	JGT subtrair								; repete enquanto o número for maior que o número máximo de colunas
	POP R7
	POP R6
	RET
...
limites_novo_direita:					
	...
	JMP limites_novo_ret						; entao nao precisa de testar mais pi

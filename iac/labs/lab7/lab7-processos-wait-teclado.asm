; *********************************************************************************
; * IST-UL
; * Modulo:    lab7-processos-wait-teclado.asm
; * Descrição: Este programa ilustra a diretiva WAIT, que permite otimizar o varrimento do teclado,
; *			que consome bastante tempo do processador que está a executar o simulador.
; *			Tal deve-se ao facto de que o PEPE-16 tem de estar continuamente a varrer o teclado,
; *			pois não sabe quando é o utilizador vai carregar numa tecla.
; *			Este programa é idêntico ao lab7-processos-boneco-displays-teclado.asm,
; *			com a diferença de que a primeira diretiva YIELD no processo teclado (no ciclo que espera
; *			que uma tecla seja carregada) é substituída pela diretiva WAIT.
; *			Esta diretiva faz o processador adormecer quando todos os restantes processos
; *			estão bloqueados (em LOCKs) e acordar automaticamente quando há um evento relevante
; *			(por exemplo, carregar numa tecla do teclado ou uma interrupção).
; *			Desta forma, não gasta tempo do computador que corre o simulador, exceto
; * 			quando sucede algo que precisa da atenção do PEPE-16.
; * 			NOTA - Não adianta substituir o segundo YIELD por WAIT, pois nessa altura o WAIT
; * 			comporta-se como YIELD (não pode adormecer o processador porque o utilizador

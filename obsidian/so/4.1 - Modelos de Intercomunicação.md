...
- Para mandar um sinal podemos usar:

```c
int kill(pid_t pid, int sig);
```

- `pid` é o identificador do processo e o `sig` o do sinal.
- Se o `pid` for zero é enviado a todos os processos do grupo. Isto é uma operação apenas de _superusers_.

```c
unsigned alarm (unsigned int segundos); // o signal SIGALRM é enviado para o processo depois de ocorrem esses segundos

int pause(void); // aguarda um sinal
...
### Diferentes Semânticas de Signals

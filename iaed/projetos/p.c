...
void newAirport(Airport airportList[])
{
    ...
    */
    ...
    while (id_aux[counter] != '\0')
    {
        ...
        if (id_aux[counter] > '0' && id_aux[counter] < '9')
        {
            printf("invalid airport ID\n");
            return;
        }
    ...
    if (counter > MAX_C_ID || counter < MIN_C_ID)
    {
        printf("invalid airport ID\n");
        return;
    }
...
void departList()
{
    ...
    for (i = 0; i < depart_counter; i++)
    {
        if ((currentDate.year == departList_aux[i].departDate.year) &&
            (currentDate.month == departList_aux[i].departDate.month) &&
            (currentDate.day == departList_aux[i].departDate.day))
        {
...
/* gcc -Wall -Wextra -Werror -ansi -pedantic -o ola pp.c */